...
export default function PrescriptionNonface({ route }) {
  ...
  const handleLoad = (userId, data) => {
    navigation.navigate("PrescriptionInfoStack", {
      screen: "PrescriptionNonfaceDetails",
      id: 3,
  ...
  const fetchData = async () => {
    try {
      ...
      setitem(data.data.noncontactPrescriptionItemList);
        ...
        <View style={styles.row}>
          {cnt === 0 ? (
            ...
            <ScrollView style={styles.scrollView}>
              {item.map((item) => {
                return (
                  <View key={item.noncontactPrescriptionId} style={styles.hospitalBlock}>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.timeText}>{item.prescriptionDate}</Text>
